...
import time  #used for time-related funstions like delays and timestamps  #used for downloading contents from instagram
...
def speak(audio):
    engine = pyttsx3.init('sapi5')   # create engine inside current thread
...
class MainThread(QThread):
    ...
    def authenticate_user(self):
        ...
        name_response = self.takeCommand().lower().strip()
        print("Heard:", name_response)

        # Check if the recognized name is in the authorized users dictionary
        if name_response in authorized_user:
            speak("Speak your passcode")
            pass_response = self.takeCommand().lower()
            print("Passcode heard:", pass_response)
            
            # Check if the passcode matches
            if pass_response == authorized_user[name_response]:
                speak("Access granted. Hello, " + name_response)
                time.sleep(1)
                return True
            else:
